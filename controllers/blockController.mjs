...
const getBlockchain = (req, res, next) => {
  res
    ...
    .json({
      ...
      message: "Blockchain fetch successfull!",
...
const getIdBlock = (req, res, next) => {
  ...
  res
    ...
    .json({
      statusCode: res.statusCode,
      message: `Block with id ${id} recieved!`,
      data: block,
    });
};
const mineBlock = async (req, res, next) => {
  ...
  if (Object.keys(data).length !== 0) {
    ...
    next(res.status(500).json(new Error("The request you sent was empty...")));
...
const updateChain = (req, res, next) => {
  ...
  if (hash && id) {
    ...
    res
      .status(201)
      .json({ statusCode: 201, message: "Chain updated", data: block });
  } else {
    console.log("Block rejected");
    res
      .status(500)
      .json({ success: false, statusCode: 500, data: { message: "Rejected" } });
  }
};
...
const syncChain = (req, res, next) => {
  ...
  res
    .status(200)
    .json({ successCode: 200, data: { message: "Chains are synced." } });
};
